...
class HistoricTweetSource(startDate: String, keyWord: String)
    extends SourceFunction[Tweet]
  ...
  def getTwitter(cb: ConfigurationBuilder) = {
    ...
    while (numTweetsSearched != 0) {
      ...
      if (remainingLimit == 0) {
        while (twitter
                 ...
                 .getRemaining == 0) {
          println(
            s"******* API LIMIT REACHED, sleeping for time : ${searchTweetsLimit.getSecondsUntilReset}")
          Thread.sleep((searchTweetsLimit.getSecondsUntilReset + 5) * 1000)
        }
        remainingLimit = twitter
          .getRateLimitStatus("search")
          .get("/search/tweets")
          .getRemaining
      }
      ...
      val tweetsSearched =
        callTwitterSearchAPI(twitter, keyWord, maxIdToSearch, startDate)
      remainingLimit = remainingLimit - 1
      val lowestIdAndTweets = processAndGetNextMaxId(
        tweetsSearched.asScala.toList)
...
//    isRunning = false
  ...
  def getConfigBuilder() = {
    ...
    authProps.load(
      new FileInputStream(getClass.getResource("/twitter.properties").getPath))
  ...
  /**
    ...
    */
  def callTwitterSearchAPI(twitter: Twitter,
                           queryString: String,
                           maxID: Long,
                           timeSince: String) = {
